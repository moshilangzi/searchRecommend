...
public class EsUpdateService implements UpdateService {
  ...
  private static void populateClient(String elasticSearchUrl) {
    if (client == null) {
      ...
      HttpClientConfig clientConfig =
          new HttpClientConfig.Builder(servers).multiThreaded(true).connTimeout(6 * 1000)
              .readTimeout(6 * 1000).defaultMaxTotalConnectionPerRoute(1000).build();
  ...
  public static void main(String[] args) throws Exception {
    ...
    HttpClientConfig clientConfig = new HttpClientConfig.Builder(servers).multiThreaded(true)
        .connTimeout(6 * 1000).readTimeout(6 * 1000).defaultMaxTotalConnectionPerRoute(1000)
        .defaultCredentials("zhaoyufei", "zhaoyufei").build();

    JestClientFactory factory = new JestClientFactory();
    factory.setHttpClientConfig(clientConfig);
    client = factory.getObject();

    DeleteByQuery deleteAllUserJohn =
        new DeleteByQuery.Builder("\"query\" : {\n" + "        \n" + "    \"range\" : {\n"
            + "        \"accept_date\" : {\n" + "            \"lt\" :  \"now-7d/d\",\n"
            + "            \"gte\": \"now-8d/d\"\n" + "        }\n" + "    \n" + "}\n" + "    }")
                .addIndex("baihe").addType("ha").build();
  ...
  private void checkParameters(Map<String, String> updateMap) {
    ...
    if (type == null || index == null) {
      throw new IllegalArgumentException(
          "updated Info must contains id, type, index field infomation!");
    }
   ...
   * @param updatedMaps once failure throw exception must to be processed by caller
   */
  ...
  /**
   * used to update or save a document, the input document must have id field, means the document's
   * id in elasticserch must be manually specify.
   * @param updateMap
   */
