...
public class SolrUpdateService implements UpdateService {
  ...
  private void populateCloudServers() throws MalformedURLException {
    ...
    for (String collectionNames_1 : collectionNames_) {
      ...
      int max_connections_per_host =
          Integer.parseInt(solrProperties.getString("solrcloud.max_connections_per_host"));
      int zkConnectTimeout =
          Integer.parseInt(solrProperties.getString("solrcloud.zkConnectTimeout"));
  ...
  /**
   * @param updateMap make sure dateformat: yyyy-MM-ddTHH:mm:ssZ make sure updateMap contains
   *          collectionName of solr you want to update if unique key fieldName==id, then contains
   *          id value, if not, then put the unique key fieldName as idKey, at same time contains
   *          unique key value
   */
  @Override
