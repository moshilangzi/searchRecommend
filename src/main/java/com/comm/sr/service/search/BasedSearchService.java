 ...
 */
public abstract class BasedSearchService<A extends AbstractQueryService, Q extends CommonQuery, SR extends SearchServiceRule, R2>
    extends AbstractComponent implements Serializable {
  ...
  private void doMakeFinalQuery(Q query) {
    ...
    if (sortStr != null && sortStr.trim().length() > 0) {
      List<SortItem> sortItems_ =
          (List<SortItem>) GsonHelper.jsonToObj(sortStr, new TypeToken<ArrayList<SortItem>>() {
          }.getType());
  ...
  public R2 search(String queryStr) {
    ...
    try {
      ...
      topicService.publishTopicMessage(settings.getProperty("searchTopicName", "srTopic"),
