...
public class BHSRAction {
  ...
  final static ThreadLocal<ThreadShardEntity> threadShardEntity =
    ...
    try {
      if (params != null && !params.trim().isEmpty()) {
        Map<String, String> paraMap =
            JSON.parseObject(params, new TypeReference<Map<String, String>>() {
            });
  ...
  public void clearAppRuleCache(HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    // get appkey, profile user id, visited user id
    // get recommend service using appley, then call service to return Map result
    // format map result to response
  ...
  private void printJsonTemplate(int code, String msg, Object data, HttpServletRequest request,
    ...
    try {
      ...
      LOGGER.info("error to access threadShardEntity, message is: "
