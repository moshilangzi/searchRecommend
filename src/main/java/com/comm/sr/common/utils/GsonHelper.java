...
public class GsonHelper {
  ...
  public static void main(String[] args) {
    ...
    List<QueryItem> items1 =
        (List<QueryItem>) GsonHelper.jsonToObj(listStr, new TypeToken<ArrayList<QueryItem>>() {
        }.getType());
    ...
    List<SubQuery> subQueries = Lists.newArrayList(
      new SubQuery("AND", new QueryItem("des", Lists.newArrayList("basket", "football"))));
    ...
    // EsCommonQuery baiheQuery = new EsCommonQuery(items, 1, 18, sortItems, fls, "baihe_user",
    // "user");
    ...
    EsCommonQuery esCommonQuery =
        (EsCommonQuery) GsonHelper.jsonToObj(jsonStr, EsCommonQuery.class);
