...
public class JestMain {
  ...
  final static String esMasterUrl =
      ResourceBundle.getBundle("elasticSearch").getString("elasticSearchUrl");

  public static JestClient client = null;

  static {
    HttpClientConfig clientConfig =
        new HttpClientConfig.Builder(esMasterUrl).multiThreaded(true).build();
  ...
  public static void commTest() throws IOException {
    // String source =XContentFactory.jsonBuilder().startObject().field("height", 198).field("age",
    // 1988).endObject().toString();
  ...
  public static void testUpdateDate() throws IOException, Exception {
    ...
    // EsCommonQuery escommQuery = new EsCommonQuery(queryItems, 1, 10, Lists.newArrayList(),
    // Lists.newArrayList("age", "height"), "comm", "user");
    ...
    String updateScript =
        "{\"script\" : \"ctx._source.nickname=nickname;ctx._source.city=city; ctx._source.height=height;ctx._source.age=age\",\"params\" : {\"city\": 864102,\"age\":1988,\"height\":199, \"nickname\":\"jasstion1\"},\"upsert\":{\"id\":\"99999\"}}";//
    Update update =
        new Update.Builder(updateScript).index("comm").type("user").id("111111").build();
